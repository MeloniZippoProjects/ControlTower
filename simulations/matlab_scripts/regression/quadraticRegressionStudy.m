function [ mdl ] = quadraticRegressionStudy(evaluationPoints, responses, subjectName, rootFolderName)
    ...
    transformedPoints = positiveEvaluationPoints.^4;
	foldername = fullfile(rootFolderName, 'quadraticRegression', subjectName);
	mdl = linearRegressionStudy(transformedPoints, positiveResponses, subjectName, foldername);
	
	% Anti-transform
	beta = mdl.Coefficients.Estimate;
    syms x;
	quadraticRegression = beta(1) + beta(2)*(x^4);
    
	regrPlotName = 'Quadratic regression';
	...
	target_ylim = ylim;
	target_xlim = xlim;
	fplot(quadraticRegression);
	ylim(target_ylim);
	xlim(target_xlim);

	% Fitting annotation
	formula = char(quadraticRegression);
